...
IntGauge IntGaugeFamily::GetOrAdd(Span<const LabelView> labels)
	...
	return IntGauge{
		m->CreateInt64UpDownCounter(std::string{name}, std::string{helptext}, std::string{unit}),
		labels};
	}
...
DblGauge DblGaugeFamily::GetOrAdd(Span<const LabelView> labels)
	...
	auto m = p->GetMeter(std::string{prefix});
	return DblGauge{
		m->CreateDoubleUpDownCounter(std::string{name}, std::string{helptext}, std::string{unit}),
		labels};
	}
